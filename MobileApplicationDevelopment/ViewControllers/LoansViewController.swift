...
class LoansViewController: UIViewController, UITextFieldDelegate {
    ...
    @IBOutlet weak var monthsText: UITextField!
    ...
    override func viewDidLoad() {
        ...
        if (viewOpened){
            ...
            let loanAmountStored = userSavedData.string(forKey: "loanAmountLoan")
            let interestStored = userSavedData.string(forKey: "interestLoan")
            let monthlyPaymentStored = userSavedData.string(forKey: "paymentLoan")
            let monthsStored = userSavedData.string(forKey: "durationLoan")
    ...
    @IBAction func calculateButton(_ sender: Any) {
        ...
        let years: Double! = Double(monthsText.text!)
        if(years == nil) {
            count += 1
        ...
        if (years == 0){
            TextfieldFormatting.generalErrorValidation(textfield: monthsText, title: pageTitle, errorCode: 2, helpBtn: helpButton)
        ...
        case CheckFilledTextfield.loanAmount:
            let loanAmount = Loan.loanAmount(payment: monthlyPayment, interest: interest, year: years / 12)
        ...
        case CheckFilledTextfield.monthlyPayment:
            let monthlyPayment = Loan.monthlyPayment(loanAmount: loanAmount, interest: interest, year: years / 12)
            monthlyPaymentText.text = String(format: " £ %.2f",monthlyPayment)
        ...
        case CheckFilledTextfield.years:
            ...
            monthsText.text = String(format: "%.1f",loanDuration * 12 )
            TextfieldFormatting.greenBackgroundTexfield(textfield: monthsText)
        default:
            return
        }
    }
    ...
    @objc func saveData(){
        userSavedData.set(self.loanAmountText.text, forKey: "loanAmountLoan")
        userSavedData.set(self.interestText.text, forKey: "interestLoan")
        userSavedData.set(self.monthlyPaymentText.text, forKey: "paymentLoan")
        userSavedData.set(self.monthsText.text, forKey: "durationLoan")
    }
